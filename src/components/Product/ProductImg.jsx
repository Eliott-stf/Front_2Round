...
export default function ProductImg({ product }) {
  ...
  return (
    <div className="relative isolate w-full max-w-133.75 h-135.25 bg-[#111111] shrink-0 overflow-hidden group select-none">
      ...
      {images.length > 1 && (
        <>
          <button
            onClick={handlePrev}
            className="absolute z-10 left-6 top-1/2 -translate-y-1/2 p-2 text-white/80 drop-shadow-md hover:text-white active:text-gray-500 opacity-0 group-hover:opacity-100 transition-all duration-200 outline-none"
          >
          ...
          <button
            onClick={handleNext}
            className="absolute z-10 right-6 top-1/2 -translate-y-1/2 p-2 text-white/80 drop-shadow-md hover:text-white active:text-gray-500 opacity-0 group-hover:opacity-100 transition-all duration-200 outline-none"
          >
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="absolute z-10 top-6 right-6 px-3 py-1.5 rounded-full bg-black/50 backdrop-blur-md text-white font-inter text-[12px] font-medium tracking-wider">
            {activeIndex + 1} / {images.length}
          </div>

          <div className="absolute z-10 bottom-6 left-1/2 -translate-x-1/2 flex gap-1.5 dynamic-dots">
            {images.map((_, index) => (
              <button
                key={index}
      ...
      <button
        ...
        className="absolute z-10 bottom-5.5 right-5.5 transition-transform hover:scale-110 outline-none"
      >
